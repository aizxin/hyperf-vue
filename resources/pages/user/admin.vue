    ...
    <div id="tableExample">
        ...
        <div class="filter-container">
            <div class="left">
                <div class="filter-item">
                    <el-input v-model="listQuery.searchContent" clearable auto-complete="off" placeholder="标题"
                              @keyup.enter.native="getTableList" @clear="getTableList">
        ...
        <div class="table-container">
            <el-table :data="tableJson" style="width: 100%" border fit :header-cell-style="{'text-align': 'center'}"
                      v-loading="loading" element-loading-text="加载中..." :height="tableHeight > 300 ? tableHeight : 300">
                    ...
                    <template slot-scope="scope">
                        <el-popover trigger="hover" placement="top">
                            <template v-if="scope.row.sound.song_info">
                                <p v-if="scope.row.sound.song_info.name">{{scope.row.sound.song_info.name.type}}: {{
                                    scope.row.sound.song_info.name.name}}</p>
                                <p v-if="scope.row.sound.song_info.author">{{scope.row.sound.song_info.author.type}}: {{
                                    scope.row.sound.song_info.author.name}}</p>
                                <p v-if="scope.row.sound.song_info.album_name">
                                    {{scope.row.sound.song_info.album_name.type}}: {{
                                    scope.row.sound.song_info.album_name.name}}</p>
                            </template>
                            <div slot="reference">
                ...
                <el-table-column align="center" label="操作" width="200" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="primary" icon="el-icon-edit"
                                   @click="handleDialogVisible(scope.row, 'editVisible')">编辑
                        </el-button>
                        <el-button type="danger" icon="el-icon-delete" @click="deleteAction(scope.row, scope.$index)">
                            删除
                        </el-button>
                    </template>
                </el-table-column>
        ...
        <div class="pagination-container">
            <div class="left">
                <el-pagination @current-change="handleCurrentChange" :current-page="listQuery.currentPage"
                               :page-size="listQuery.pageSize" layout="total, sizes, prev, pager, next, jumper"
                               :total="total" :page-sizes="[10, 20, 50, 100]" @size-change="handleSizeChange">
                </el-pagination>
            </div>
        </div>
        <!-- 弹框 -->
        <app-dialog title="新增数据" v-if="createVisible" :visible.sync="createVisible" @onSubmit="createAction"/>
        <app-dialog title="编辑数据" v-if="editVisible" :visible.sync="editVisible" @onSubmit="editAction" :json="editItem"
                    disabled/>
    </div>
</template>
<script>
